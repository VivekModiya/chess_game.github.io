...
function get_black_pawn_moves(board, id) {
    ...
    if (y + 1 < 8 && board.white_double == game.get_id(x, y + 1)) {
        ...
        else moves.push(game.get_id(x + 1, y + 1));
...
function get_legal_moves(board, id, checked_cells) {
    ...
    if (peice == null) {
        ...
        if (
            board.is_black_king_moved == 0 &&
            game.is_check_to_black(board, 'A5').length == 0
        ) {
            if (
                board.is_black_right_rock_moved == 0 &&
                game.get_peice(board, 'A6') == null &&
                game.get_peice(board, 'A7') == null &&
                game.is_check_to_black(board, 'A7').length == 0 &&
                game.is_check_to_black(board, 'A6').length == 0
            ) {
                safe_cells.push('A7');
            }
            if (
                board.is_black_left_rock_moved == 0 &&
                game.get_peice(board, 'A2') == null &&
                game.get_peice(board, 'A3') == null &&
                game.get_peice(board, 'A4') == null &&
                game.is_check_to_black(board, 'A2').length == 0 &&
                game.is_check_to_black(board, 'A3').length == 0 &&
                game.is_check_to_black(board, 'A4').length == 0
            ) {
                safe_cells.push('A3');
            }
        }
        ...
        if (
            board.is_white_king_moved == 0 &&
            game.is_check_to_white(board, 'H5').length == 0
        ) {
            if (
                board.is_white_right_rock_moved == 0 &&
                game.get_peice(board, 'H6') == null &&
                game.get_peice(board, 'H7') == null &&
                game.is_check_to_white(board, 'H7').length == 0 &&
                game.is_check_to_white(board, 'H6').length == 0
            ) {
                safe_cells.push('H7');
            }
            if (
                board.is_white_left_rock_moved == 0 &&
                game.get_peice(board, 'H2') == null &&
                game.get_peice(board, 'H3') == null &&
                game.get_peice(board, 'H4') == null &&
                game.is_check_to_white(board, 'H2').length == 0 &&
                game.is_check_to_white(board, 'H3').length == 0 &&
                game.is_check_to_white(board, 'H4').length == 0
            ) {
                safe_cells.push('H3');
            }
        }
...
function make_move(board, last_clicked, id, depth) {
    ...
    if (peice == black_pawn) {
        ...
        if (
            (game.has_black_peice(board, right_down) ||
                game.has_black_peice(board, left_down)) &&
            game.get_peice(board, right_down) != black_king &&
            game.get_peice(board, left_down) != black_king
        ) {
            board.point += 0.3;
        }
        if (x - xlast > 1) {
    ...
    if (peice == white_pawn) {
        ...
        let right_down = game.get_peice(
            board,
            game.get_id(xlast + 1, ylast + 1)
        );
        let left_down = game.get_peice(
            board,
            game.get_id(xlast + 1, ylast - 1)
        );
        ...
        if (
            (game.has_white_peice(board, right_up) ||
                game.has_white_peice(board, left_up)) &&
            game.get_peice(board, right_up) != white_king &&
            game.get_peice(board, left_up) != white_king
        ) {
            board.point += 0.3;
        }
        if (xlast - x > 1) {
    ...
    if (game.has_white_peice(newBoard, id)) {
        let checked_cells = game.is_check_to_black(
            newBoard,
            newBoard.black_king
        );
        ...
        let checked_cells = game.is_check_to_white(
            newBoard,
            newBoard.white_king
        );
    ...
    if (peice == black_rock) {
        if (last_clicked == 'A1') newBoard.is_black_left_rock_moved = 1;
        else newBoard.is_black_right_rock_moved = 1;
    }
    if (peice == black_king) {
    ...
    if (peice == white_rock) {
        if (last_clicked == 'A1') newBoard.is_white_left_rock_moved = 1;
        else newBoard.is_white_right_rock_moved = 1;
    }
    if (peice == white_king) {
            ...
            if (game.has_black_peice(board, id)) {
                ...
                if (moves.length > 0) {
                    moves.map((element) => {
                        ...
                        let blackPoint = think_for_white(
                            newBoard,
                            depth - 1
                        )[2];
...
onmessage = (board) => {
    let moves = think_for_black(board.data, 9);
    postMessage(moves);
};
