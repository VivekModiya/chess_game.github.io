...
function clear_background(chessBoard) {
    for (let id of chessBoard.bg_highlighted_color.keys()) {
        document
            .getElementById(id)
            .children[2].children[0].setAttribute('hidden', 'hidden');
        document
            .getElementById(id)
            .children[2].children[1].setAttribute('hidden', 'hidden');
        document
            .getElementById(id)
            .children[3].setAttribute('hidden', 'hidden');
    }

    for (let id of chessBoard.shown_moves) {
        document
            .getElementById(id)
            .children[1].children[0].setAttribute('hidden', 'hidden');
    }
}
function set_blue_bg(chessBoard, id) {
    document
        .getElementById(id)
        .children[2].children[0].removeAttribute('hidden');
...
function set_red_bg(chessBoard, id) {
    document
        .getElementById(id)
        .children[2].children[1].removeAttribute('hidden');
...
function show_moves(chess, id) {
    document
        .getElementById(id)
        .children[1].children[0].removeAttribute('hidden');
}
...
function set_peice(id, peice) {
    ...
    document
        .getElementById(id)
        .children[0].children[0].setAttribute('src', peice);
}
function show_last_move(id) {
    document
        ...
        .children[0].setAttribute(
            'style',
            'box-shadow: 0.1vmin 0.1vmin 0.6vmin rgb(110, 0, 228) inset, -0.1vmin -0.1vmin 0.6vmin rgb(110, 0, 228) inset'
        );
}
function remove_last_move(id) {
    document
        ...
        .children[0].setAttribute(
            'style',
            'box-shadow: 0vmin 0vmin 0vmin rgb(110, 0, 228) inset, 0vmin 0vmin 0vmin rgb(110, 0, 228) inset'
        );
}
function pawn_promotion(chess, id) {
    ...
    if (game.get_position(id)[0] == 7) {
        element.children[0].children[0].children[0].setAttribute(
            'src',
            black_queen
        );
        element.children[0].children[1].children[0].setAttribute(
            'src',
            black_rock
        );
        element.children[0].children[2].children[0].setAttribute(
            'src',
            black_knight
        );
        element.children[0].children[3].children[0].setAttribute(
            'src',
            black_bishop
        );
    } else {
    ...
    let Children = Array.from(
        document.getElementsByClassName('pawn_promotion')[0].children
    );
...
function drag_peice(event, target) {
    ...
    document.onmousemove = (ev) => {
        ...
        let boardTop = board.offsetTop,
            ...
            boardBottom =
                board.offsetTop + board.clientHeight - target.clientHeight,
            boardRight =
                board.offsetLeft + board.clientWidth - target.clientHeight;
...
function handle_click(event, chess) {
    event.preventDefault();
    if (!(chess.term == 1 && chess.player == 'computer')) {
        let target = event.target;
        while (!target.id) {
            target = target.parentNode;
        }
        ...
        if (!chess.shown_moves.includes(id)) {
            game.clear_background(chess);
            if (
                (chess.term == 0 && game.has_white_peice(chess.board, id)) ||
                (chess.term == 1 && game.has_black_peice(chess.board, id))
            ) {
