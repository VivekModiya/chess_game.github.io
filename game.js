...
function undo(chess, cnt = true) {
    ...
    if (last_moves['1'][1] != null) {
        ...
        dom.remove_from_captured(
            last_moves['1'][1] >= 'A' && last_moves['1'][1] <= 'Z'
                ? 'black'
                : 'white'
        );
    } else {
...
function is_check_to_white(board, id) {
    ...
    moves.map((id1) => {
        ...
        if (
            has_black_peice(board, id1) &&
            (peice == black_bishop || peice == black_queen)
        ) {
            let temp = [];
            temp.peice = peice;
    ...
    moves.map((id1) => {
        ...
        if (
            has_black_peice(board, id1) &&
            (get_peice(board, id1) == black_rock ||
                get_peice(board, id1) == black_queen)
        ) {
            let temp = [];
            temp.peice = peice;
            let x1 = get_position(id1)[0],
    ...
    moves.map((id1) => {
        ...
        if (
            has_black_peice(board, id1) &&
            get_peice(board, id1) == black_knight
        ) {
    ...
    moves.map((id1) => {
        ...
        if (
            has_black_peice(board, id1) &&
            get_peice(board, id1) == black_king
        ) {
    ...
    if (
        x - 1 >= 0 &&
        y + 1 < 8 &&
        get_peice(board, get_id(x - 1, y + 1)) == black_pawn
    ) {
...
function is_check_to_black(board, id) {
    ...
    moves.map((id1) => {
        let peice = get_peice(board, id1);
        if (
            has_white_peice(board, id1) &&
            (peice == white_bishop || peice == white_queen)
        ) {
            let temp = [];
            temp.peice = peice;
            let x1 = get_position(id1)[0],
    ...
    moves.map((id1) => {
        ...
        if (
            has_white_peice(board, id1) &&
            (get_peice(board, id1) == white_rock ||
                get_peice(board, id1) == white_queen)
        ) {
            let temp = [];
            temp.peice = peice;
            let x1 = get_position(id1)[0],
    ...
    moves.map((id1) => {
        if (
            has_white_peice(board, id1) &&
            get_peice(board, id1) == white_knight
        ) {
            let temp = [id1];
    ...
    moves.map((id1) => {
        if (
            has_white_peice(board, id1) &&
            get_peice(board, id1) == white_king
        ) {
            let temp = [id1];
    ...
    if (
        x + 1 < 8 &&
        y + 1 < 8 &&
        get_peice(board, get_id(x + 1, y + 1)) == white_pawn
    ) {
        let temp = [get_id(x + 1, y + 1)];
...
function get_black_pawn_moves(chess, id) {
    ...
    if (y + 1 < 8 && chess.white_double == get_id(x, y + 1)) {
        ...
        else moves.push(get_id(x + 1, y + 1));
...
function get_white_knight_moves(board, id) {
    ...
    possible_moves.map((xy) => {
        ...
        if (
            ...
            x1 < 8 &&
            y1 < 8 &&
            !has_white_peice(board, get_id(x1, y1))
        ) {
...
function get_black_knight_moves(board, id) {
    ...
    possible_moves.map((xy) => {
        ...
        if (
            ...
            x1 < 8 &&
            y1 < 8 &&
            !has_black_peice(board, get_id(x1, y1))
        ) {
            moves.push(get_id(x1, y1));
        }
    });
...
function get_legal_moves(chess, id, checked_cells) {
    ...
    if (peice == null) {
        ...
        if (checked_cells.length == 0) {
            ...
            moves = possible_moves.filter((value) =>
                checked_cells[0].includes(value)
            );
        }
        ...
        if (
            chess.is_black_king_moved == 0 &&
            is_check_to_black(board, 'A5').length == 0
        ) {
            if (
                chess.is_black_right_rock_moved == 0 &&
                get_peice(board, 'A6') == null &&
                get_peice(board, 'A7') == null &&
                is_check_to_black(board, 'A7').length == 0 &&
                is_check_to_black(board, 'A6').length == 0
            ) {
                safe_cells.push('A7');
            }
            if (
                chess.is_black_left_rock_moved == 0 &&
                get_peice(board, 'A2') == null &&
                get_peice(board, 'A3') == null &&
                get_peice(board, 'A4') == null &&
                is_check_to_black(board, 'A2').length == 0 &&
                is_check_to_black(board, 'A3').length == 0 &&
                is_check_to_black(board, 'A4').length == 0
            ) {
                safe_cells.push('A3');
            }
        }
        ...
        set_peice(board, id, peice);
        if (
            chess.is_white_king_moved == 0 &&
            is_check_to_white(board, 'H5').length == 0
        ) {
            if (
                chess.is_white_right_rock_moved == 0 &&
                get_peice(board, 'H6') == null &&
                get_peice(board, 'H7') == null &&
                is_check_to_white(board, 'H7').length == 0 &&
                is_check_to_white(board, 'H6').length == 0
            ) {
                safe_cells.push('H7');
            }
            if (
                chess.is_white_left_rock_moved == 0 &&
                get_peice(board, 'H2') == null &&
                get_peice(board, 'H3') == null &&
                get_peice(board, 'H4') == null &&
                is_check_to_white(board, 'H2').length == 0 &&
                is_check_to_white(board, 'H3').length == 0 &&
                is_check_to_white(board, 'H4').length == 0
            ) {
                safe_cells.push('H3');
            }
        }
...
async function make_move(chess, id, isAuto = true) {
    ...
    let len = chess.moves_queue.length,
        last_move;
    if (len) last_move = chess.moves_queue[len - 1];
    ...
    if (peice == black_pawn) {
        if (
            id_peice == null &&
            get_position(id)[1] - get_position(last_clicked)[1] != 0
        ) {
            saved_move['3'] = [
                get_id(get_position(id)[0] - 1, get_position(id)[1]),
                white_pawn,
            ];
        }
    }
    ...
    set_peice(board, id, peice);
    if (isAuto) await dom.play_move(peice, last_clicked, id);
    if (id_peice != null) {
    ...
    if (peice == black_rock) {
        if (last_clicked == 'A1') chess.is_black_left_rock_moved = 1;
        else chess.is_black_right_rock_moved = 1;
    }
    if (peice == black_king) {
    ...
    if (peice == white_rock) {
        if (last_clicked == 'H1') chess.is_white_left_rock_moved = 1;
        else chess.is_white_right_rock_moved = 1;
    }
    if (peice == white_king) {
