...
computer.onclick = () => {
    game.flip_player(chess, 'computer');
};
friend.onclick = () => {
    game.flip_player(chess, 'friend');
};

const chess_board = document.querySelector('.board');
chess_board.addEventListener('mousedown', (event) =>
    dom.handle_click(event, chess)
);
chess_board.addEventListener('mouseup', (event) =>
    dom.drop_peice(event, chess)
);
chess_board.addEventListener('mouseover', (event) =>
    dom.change_to_grab(event, chess)
);
document
    .getElementsByClassName('undo')[0]
    .addEventListener('click', () => game.undo(chess));
